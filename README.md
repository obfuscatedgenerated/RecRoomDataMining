...
`descriptor_set.binpb` is the protobuf schema shipped in every Rec Room data export. It describes the format of all the export's `.binpb` files. The `proto/` folder holds the same schema as readable `.proto` files, generated with `tools/descriptor_to_proto.py`.
